...
def read_sparse_matrix_list_config(config, size):
...
def download_list(config, size):
    processed_suite_sparse_matrix_list = read_sparse_matrix_list_config(config, size)
    error_matrices = []
        ...
        for matrix in matrices:
            if matrix.name == matrix_name:
                ...
                if os.path.isdir(f"{datasets_dir_path}/{full_name}"):
                    ...
                    print(100 * "=")
                    matrix.download(destpath=matrix_location, extract=True)
                    print(100 * "=")
...
def read_sparse_matrix_range_config(config, size):
    if not size in config:
        raise Exception(
            f"{colors.color_red('size')} key is not configured properly. Refer to the config.example.yaml file"
        )

    if not "suite_sparse_matrix_range" in config[size]:
        raise Exception(
            f"{colors.color_red('suite_sparse_matrix_range')} key is not configured properly. Refer to the config.example.yaml file"
        )

    if not "min_nnzs" in config[size]["suite_sparse_matrix_range"]:
        raise Exception(
            f"{colors.color_red('min_nnzs')} key is not configured properly. Refer to the config.example.yaml file"
        )

    min_nnzs = config[size]["suite_sparse_matrix_range"]["min_nnzs"]
    ...
    if not "max_nnzs" in config[size]["suite_sparse_matrix_range"]:
        raise Exception(
            f"{colors.color_red('max_nnzs')} key is not configured properly. Refer to the config.example.yaml file"
        )

    max_nnzs = config[size]["suite_sparse_matrix_range"]["max_nnzs"]
    ...
    if not "limit" in config[size]["suite_sparse_matrix_range"]:
        raise Exception(
...
def download_range(config, size):
    (min_nnzs, max_nnzs, limit) = read_sparse_matrix_range_config(config, size)
    ...
    for matrix in matrices:
        matrix_location = f"{datasets_dir_path}/{matrices_dir_name}/{matrix.group}"
        full_name = f"{matrix.group}/{matrix.name}"
        if os.path.isdir(f"{matrix_location}/{matrix.name}"):
            print(f"\t\t{colors.color_yellow(full_name)} was already founded, skipped")
