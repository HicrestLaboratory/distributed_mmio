...
#define MMIO_UTILS_EXPLICIT_TEMPLATE_INST(IT, VT) \
  template void DMMIO_print_CSR(CSR<IT, VT> *csr, std::string header, FILE* fp); \
  template void DMMIO_print_CSR_as_dense<IT, VT>(CSR<IT, VT> *csr, std::string header, FILE* fp); \
  template void DMMIO_print_COO(COO<IT, VT> *coo, std::string header, FILE* fp); \
  template void DMMIO_print_COO_as_dense(COO<IT, VT> *coo, std::string header, FILE* fp);
...
void DMMIO_print_CSR(CSR<IT, VT> *csr, std::string header, FILE* fp) {
  if (header != "") {
    fprintf(fp, "%s -- ", header.c_str());
  }
  std::string I_FMT = "%3u";
...
template<typename IT, typename VT>
void DMMIO_print_COO(COO<IT, VT> *coo, std::string header, FILE* fp) {
  if (header != "") {
    fprintf(fp, "%s -- ", header.c_str());
  }
  std::string I_FMT = "%4u";
...
template<typename IT, typename VT>
void DMMIO_print_CSR_as_dense(CSR<IT, VT> *csr, std::string header, FILE* fp) {
  // Build dense matrix
...
template<typename IT, typename VT>
void DMMIO_print_COO_as_dense(COO<IT, VT> *coo, std::string header, FILE* fp) {
