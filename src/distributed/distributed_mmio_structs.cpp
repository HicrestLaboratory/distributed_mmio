...
#define DMMIO_DSTRUCTS_EXPLICIT_TEMPLATE_INST(IT, VT) \
  template DCOO<IT, VT>* DMMIO_DCOO_read(const char *filename, int mpi_comm_size, int rank, int grid_rows, int grid_cols, int grid_node_size, DMMIO_PARTITIONING partitioning_type, DMMIO_OP op, bool expl_val_for_bin_mtx, DMMIO_Matrix_Metadata* meta);
...
DMMIO_Partitioning* DMMIO_Partitioning_create(int matrix_rows, int matrix_cols, int grid_rows, int grid_cols, int grid_node_size, DMMIO_PARTITIONING partitioning_type, DMMIO_OP op) {
  DMMIO_Partitioning *partitioning = (DMMIO_Partitioning*)malloc(sizeof(DMMIO_Partitioning));
  DMMIO_ProcessGrid *grid = make_process_grid(grid_rows, grid_cols, grid_node_size);

  set_mypart_type(partitioning, partitioning_type);
  set_mypart_grid(partitioning, grid);
  set_mypart_globaldim(partitioning, matrix_rows, matrix_cols);
  set_mypart_groupdim(partitioning);
  set_mypart_localdim(partitioning);
  set_mypart_functions(partitioning);

  return partitioning;
}

// TODO partitioning destroy
...
DCOO<IT, VT>* DMMIO_DCOO_read(
  ...
  DMMIO_PARTITIONING partitioning_type, DMMIO_OP op,
  bool expl_val_for_bin_mtx, DMMIO_Matrix_Metadata* meta
) {
  return DMMIO_DCOO_read_f<IT, VT>(
    open_file_r(filename),
    mpi_comm_size, rank,
    grid_rows, grid_cols, grid_node_size,
    partitioning_type, op,
    is_file_extension_bmtx(std::string(filename)), expl_val_for_bin_mtx, meta
  );
}
...
DCOO<IT, VT>* DMMIO_DCOO_read_f(
  ...
  int grid_rows, int grid_cols, int grid_node_size,
  DMMIO_PARTITIONING partitioning_type, DMMIO_OP op,
  bool is_bmtx, bool expl_val_for_bin_mtx, DMMIO_Matrix_Metadata* meta
) {
  IT nrows, ncols, nnz;
  MM_typecode matcode;
  Entry<IT, VT> *entries = mm_parse_file_distributed<IT, VT>(f, rank, mpi_comm_size, nrows, ncols, nnz, &matcode, is_bmtx, meta);
  if (entries == NULL) return NULL;
  DMMIO_Partitioning *partitioning = DMMIO_Partitioning_create(nrows, ncols, grid_rows, grid_cols, grid_node_size, partitioning_type, op);
  DCOO<IT, VT> *dcoo = (DCOO<IT, VT>*)malloc(sizeof(DCOO<IT, VT>));
  dcoo->partitioning = partitioning;

  int *owner = (int*)malloc(sizeof(int)*nnz);
  for (int i=0; i<nnz; i++) owner[i] = edge2globalprocess(partitioning, entries[i].row, entries[i].col);
  ...
  for (int i=0; i<nnz; i++) owner[i] = edge2globalprocess(partitioning, sorted_entries[i].row, sorted_entries[i].col);
...
#define DEBUG_RESULT
  ...
  for (int i=0; i<total_recv; i++) {
      dbg_entries[i].owner = edge2globalprocess(partitioning, recv_entries[i].row, recv_entries[i].col);
