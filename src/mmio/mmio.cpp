...
template <typename IT, typename VT>
using DENSE = mmio::DENSE<IT, VT>;
template <typename IT, typename VT>
using COO = mmio::COO<IT, VT>;
template <typename IT, typename VT>
using CSR = mmio::CSR<IT, VT>;
template <typename IT, typename VT>
using CSC = mmio::CSC<IT, VT>;
template <typename IT, typename VT>
using CSX = mmio::CSX<IT, VT>;
...
#define MMIO_EXPLICIT_TEMPLATE_INST(IT, VT)                                                                            \
    ...
    template CSX<IT, VT>* mmio::CSX_create(IT nrows, IT ncols, IT nnz, MajorDim majordim, IT* ptr_vec, IT* idx_vec,    \
                                           VT* val_vec);                                                               \
...
//  template CSX<IT, VT>* mmio::CSX_create_contig(IT nrows, IT ncols, IT nnz, bool alloc_val, MajorDim majordim, bool
//  device_alloc);
...
COO<IT, VT>* COO_read_f(FILE* f, bool is_bmtx, bool expl_val_for_bin_mtx, Matrix_Metadata* meta, bool remove_diagonal) {
    ...
    MM_typecode matcode;
    Entry<IT, VT>* entries =
        mmio::io::mm_parse_file<IT, VT>(f, nrows, ncols, nnz, &matcode, is_bmtx, meta, remove_diagonal);
    if (entries == NULL)
        return NULL;
...
int COO_write_f(COO<IT, VT>* coo, FILE* f, bool write_as_binary, Matrix_Metadata* meta) {
    if (meta->mm_header.empty()) {
        ...
        switch (meta->value_type) {
            case mmio::ValueType::Real: {
                meta->mm_header += std::string(MM_REAL_STR);
                break;
            }
            case mmio::ValueType::Integer: {
                meta->mm_header += std::string(MM_INT_STR);
                break;
            }
            case mmio::ValueType::Pattern: {
                meta->mm_header += std::string(MM_PATTERN_STR);
                break;
            }
            default: {
                fprintf(stderr, "BUG: ValueType not recognized\n");
                return 100;
            }
        }
    ...
    return write_as_binary ? mmio::io::mm_write_binary_matrix_market(f, coo, meta)
                           : mmio::io::mm_write_matrix_market(f, coo, meta);
}
...
template <typename IT, typename VT>
CSR<IT, VT>* CSR_read(const char* filename, bool expl_val_for_bin_mtx, Matrix_Metadata* meta, bool remove_diagonal) {
    return mmio::CSR_read_f<IT, VT>(mmio::io::open_file_r(filename),
                                    mmio::io::mm_is_file_extension_bmtx(std::string(filename)), expl_val_for_bin_mtx,
                                    meta, remove_diagonal);
}

template <typename IT, typename VT>
CSR<IT, VT>* CSR_read_f(FILE* f, bool is_bmtx, bool expl_val_for_bin_mtx, Matrix_Metadata* meta, bool remove_diagonal) {
    IT nrows, ncols, nnz;
    MM_typecode matcode;
    Entry<IT, VT>* entries =
        mmio::io::mm_parse_file<IT, VT>(f, nrows, ncols, nnz, &matcode, is_bmtx, meta, remove_diagonal);
...
CSR<IT, VT>* COO2CSR(COO<IT, VT>* coo, bool alloc_val) {
    ...
    std::sort(triples.begin(), triples.end(), [](auto& t1, auto& t2) { return t1.row < t2.row; });

    // First convert the local COO representation to a CSR representation
    std::vector<IT> rowptrs(coo->nrows + 1, 0);
    std::for_each(triples.begin(), triples.end(), [&](auto& t) { rowptrs[t.row + 1]++; });

    std::vector<IT> colinds(coo->nnz);
    std::transform(triples.begin(), triples.end(), colinds.begin(), [&](auto& t) { return t.col; });

    if (alloc_val) {
        std::vector<VT> vals(coo->nnz);
        std::transform(triples.begin(), triples.end(), vals.begin(), [&](auto& t) { return t.val; });
...
DENSE<IT, VT>* coo2dense(COO<IT, VT>* coo) {
    ...
    for (size_t i = 0; i < (coo->nnz); i++)
        M->val[(coo->row[i]) * M->ncols + (coo->col[i])] = coo->val[i];
