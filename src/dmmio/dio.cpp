...
#define DMMIO_DISTRIBUTED_EXPLICIT_TEMPLATE_INST(IT, VT)                                                               \
    template Entry<IT, VT>* dmmio::io::sort_entries_by_owner(const Entry<IT, VT>* entries, const int* owner,           \
                                                             size_t nentries);                                         \
    template Entry<IT, VT>* dmmio::io::mm_parse_file_distributed(                                                      \
        FILE* f, int rank, int mpi_comm_size, IT& nrows, IT& ncols, IT& local_nnz, MM_typecode* matcode, bool is_bmtx, \
...
ProcessGrid* ProcessGrid_create(int row_size, int col_size, int node_size) {
    ...
    CCUTILS_ASSERT((grid->global_size % (row_size * col_size) == 0),
                   "Total number of processes must be a multiple of row_size * col_size");
...
void set_partitioning_local_dim(Partitioning* self) {
    ...
    CCUTILS_ASSERT(((self->group_rows != 0) && (self->group_cols != 0)),
                   "%s call before matrix dim are set:\n\tgroup_rows: %lu\n\tgroup_cols: %lu\n", __func__,
                   self->group_rows, self->group_cols);
...
void set_partitioning_functions(Partitioning* self) {
    ...
    switch (self->type) {
        case PartitioningType::Bcycle: {
            self->edge2group =
                dmmio::partitioning::edgeowner::base::d2::blockcycle;           // Previous: edge2proc_2d_blockcycle
            self->edge2node = dmmio::partitioning::edgeowner::base::d1::naive;  // Previous: edge2proc_1d_naive
            self->globalcol2groupcol =
                dmmio::partitioning::indextransform::base::cycle;  // Previous: globalindex2localindex_cycle
            self->globalrow2grouprow =
                dmmio::partitioning::indextransform::base::cycle;  // Previous: globalindex2localindex_cycle
            self->groupcol2localcol =
                dmmio::partitioning::indextransform::base::naive;  // Previous: globalindex2localindex_naive
            self->grouprow2localrow =
                dmmio::partitioning::indextransform::base::naive;  // Previous: globalindex2localindex_naive
            break;
        }
        ...
        case PartitioningType::RowSlicing: {
            ...
            if (self->op == Operation::None) {
                self->edge2group =
                    dmmio::partitioning::edgeowner::base::d2::rowslicing;  // Previous: edge2proc_2d_rowslicing
                self->globalcol2groupcol = dmmio::partitioning::indextransform::base::naive;
                self->globalrow2grouprow = dmmio::partitioning::indextransform::base::cycle;
            } else {
                self->edge2group = dmmio::partitioning::edgeowner::base::d2::
                    rowslicing_transpose;  // Previous: edge2proc_2d_rowslicing_transpose
                self->globalcol2groupcol = dmmio::partitioning::indextransform::base::cycle;
        ...
        case PartitioningType::RowSlicingCycle: {
            ...
            self->groupcol2localcol = self->op == Operation::None ? dmmio::partitioning::indextransform::base::naive
                                                                  : dmmio::partitioning::indextransform::base::cycle;
            self->grouprow2localrow = self->op == Operation::None ? dmmio::partitioning::indextransform::base::cycle
...
template <typename IT, typename VT>
Entry<IT, VT>* mm_parse_file_distributed(FILE* f, int rank, int mpi_comm_size, IT& nrows, IT& ncols, IT& local_nnz,
    ...
    nentries = (rank < nnz_upperbound % mpi_comm_size) ? (nnz_upperbound / mpi_comm_size + 1)
                                                       : (nnz_upperbound / mpi_comm_size);

    uint32_t to_skip = (rank < nnz_upperbound % mpi_comm_size) ? (nentries * rank)
                                                               : (nentries * rank + nnz_upperbound % mpi_comm_size);
