...
/// @param meta
/// @param remove_diagonal
/// @return An "heap-allocated" array of entries. Not sorted (retain MTX file entries ordering). In case of symmetric
/// matrices, entries will be explicitly duplicated.
template <typename IT, typename VT>
...
/// @brief Computes the minimum number of bytes required to store indices that range from `0` to `maxval` (using
/// `unsigned integers`)
/// @param maxval
