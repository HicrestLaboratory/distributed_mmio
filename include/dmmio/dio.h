...
mmio::io::Entry<IT, VT>* mm_parse_file_distributed(FILE* f, int rank, int mpi_comm_size, IT& nrows, IT& ncols,
                                                   IT& local_nnz, MM_typecode* matcode, bool is_bmtx,
                                                   mmio::Matrix_Metadata* meta, bool remove_diagonal = false);

template <typename IT, typename VT>
mmio::io::Entry<IT, VT>* sort_entries_by_owner(const mmio::io::Entry<IT, VT>* entries, const int* owner,
                                               size_t nentries);

void compute_local_dims(const dmmio::Partitioning* part, uint64_t global_nrows, uint64_t global_ncols,
                        uint64_t* loc_nrows, uint64_t* loc_ncols);

void set_partitioning_type(dmmio::Partitioning* self, dmmio::PartitioningType partitioning_type,
                           dmmio::Operation operation = dmmio::Operation::None);
